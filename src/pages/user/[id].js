...
export default function User() {
  ...
  useEffect(() => {
    if (!users?.loading && users) {
      ...
      setCheckAdmin(users.data[0]?.role.name === "Admin");
    }
  }, [users]);
  ...
  const handleSubmit = (e) => {
    ...
    const userSave = {
      ...
      roleId: currentUser.roleId || users.data[0].role.id,
    };
    editUser(userSave);
  };
  ...
  const handleClick = () => {
    ...
    setCurrentUser({ ...currentUser, roleId: checkAdmin ? 2 : 1 });
  };
  ...
  return (
    <>
      <Layout
        ...
        description={"Pagina de edicion de usuario de challenge app"}
      >
        <Fragment>
